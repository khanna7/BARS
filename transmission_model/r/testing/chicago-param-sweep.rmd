...
nfected_data <- lapply(person_data, function(i)
                           i[`time of infection` == 1]
                          )
```{r, echo=FALSE}
rm(list=ls())
...
## Prevalence
  ...
  The mean overall prevalence at the end of the simulation was about 
`r round(mean(prev), 1)*100`%.  
HIV prevalence as a function of time is shown below.
...
The annual incidence rates over the course of the simulation are shown below.
  ...
  #summ_inc_per_timestep <- (summary(new_inf_per_timestep/susc_per_timestep))
   summ_inc_per_timestep <- lapply(inc_per_timestep, summary)
     ...
     list_mean_inc_per_year_df <-   do.call("rbind", lapply(list_mean_inc_per_year, as.data.frame))
     ...
     ggplot(list_mean_inc_per_year_df, aes(x=variable, y=value*365*100, color=as.factor(sim_num)))+geom_line()
