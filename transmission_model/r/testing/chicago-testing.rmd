...
```{r}
## data
net <- readRDS("../../Release/output/main_network_10000_2.RDS") #main network at the end
cas_net <- readRDS("../../Release/output/main_network_10000_2.RDS")#casual network at the end

biom_data <- read.csv("../../Release/output/biomarker_log_33.csv")
counts_data <- read.csv("../../Release/output/counts_33.csv")
inf_event_data <- read.csv("../../Release/output/infection_events_33.csv")
death_data <- read.csv("../../Release/output/death_events_33.csv")
partnership_event_data <- read.csv("../../Release/output/partnership_events_33.csv")
...
#### Mean degree. 
The target main mean degree is `r round(nedges*2/n, 2)` and the target casual mean degree is
`r round(cas_n_edges*2)/n. (Currently contains both, but eventually will contain only 1).

```{r, echo=FALSE, show='asis'}
   counts_data <- cbind(counts_data, (nedges*2/n+cas_n_edges*2/n))
   colnames(counts_data)[length(colnames(counts_data))] <- "target"
   
  ggplot(counts_data, aes(x=time))+
        geom_line(aes(y=(edge_count*2/vertex_count)))+
