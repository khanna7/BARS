...
double ViralLoadCalculator::calculateViralLoadART(InfectionParameters& infection_params, IndividualViralLoadParameters& viral_load_params) {
	double viral_load = vl_params.undetectable_viral_load;
	if (infection_params.time_since_art_init < vl_params.time_to_full_supp) {
		viral_load = viral_load_params.viral_load - viral_load_params.vl_art_traj_slope;
	}
	return viral_load;
}

double ViralLoadCalculator::calculateViralLoad(InfectionParameters& infection_params, IndividualViralLoadParameters& viral_load_params) {
	if (infection_params.art_status) {
		// AK: (art.status[i] == 1 && (art.type[i] == 1 || art.type[i] == 4)
		return calculateViralLoadART(infection_params, viral_load_params);
	} else {
		// AK: (is.na(art.status[i]) && is.na(art.type[i])) || (art.status[i] == 0 && is.na(art.type[i])
		return calculateViralLoadNoART(infection_params, viral_load_params);
	}
}

double ViralLoadCalculator::calculateViralLoadNoART(InfectionParameters& infection_params, IndividualViralLoadParameters& viral_load_params) {
	...
	//if (vl_params.greaterTimeToLateStage(infection_params.time_since_infection)) {
		if (viral_load_params.viral_load < vl_params.late_stage_viral_load) {
