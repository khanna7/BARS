...
template<typename V, typename F>
void simulate(std::shared_ptr<RInside> R, Network<V>& net, const F& attributes_setter, CondomUseAssigner& assigner, double time, std::shared_ptr<Jail> jail) {
    ...
    reset_network_edges(changes, net, idx_map, time, assigner, STEADY_NETWORK_TYPE, jail);
    ...
    reset_network_edges(changes, net, idx_map, time, assigner, CASUAL_NETWORK_TYPE, jail);
}
...
void reset_network_edges(SEXP& changes, Network<V>& net, const std::map<unsigned int, unsigned int>& idx_map,
                         double time, EdgeInit& edge_initializer, int edge_type, std::shared_ptr<Jail> jail) {
