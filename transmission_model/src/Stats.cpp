...
const std::string Counts::header(
        ...
        "pop,pop_meth,pop_crack,pop_ecstasy,jail_pop,jail_pop_meth,jail_pop_crack,jail_pop_ecstasy,"
...
void Counts::incrementVertexCount(PersonPtr p) {
    ...
    if (p->isSubstanceUser(SubstanceUseType::METH)) ++pop_meth;
    if (p->isSubstanceUser(SubstanceUseType::CRACK)) ++pop_crack;
    if (p->isSubstanceUser(SubstanceUseType::ECSTASY)) ++pop_ecstasy;
    if (p->isJailed()) {
