...
create_dt_counts <- function(filename, ranges, interv_start_at=0){
  dt_counts <- fread(filename)
  dt_counts <- dt_counts[!1]
  if(interv_start_at >0) {
    existing_rows = nrow(dt_counts)
    lines_to_exclude = interv_start_at*365
    rows_to_keep = existing_rows - lines_to_exclude
    dt_counts <- tail(dt_counts, rows_to_keep)
    tick <- rep(1:rows_to_keep)
    dt_counts$tick <- tick
  }
...
summarize_yearly_prev <- function(filename = "counts.csv", ranges, interv_start_at=0){
...
summarize_yearly_inc_inside_jail <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_inside_jail[2:.N]/uninfected_jail_pop[1:(.N-1)]))),by = ycat]
  setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
 
 summarize_yearly_inc_outside_jail <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*((infected_via_transmission[2:.N] - infected_inside_jail[2:.N])/(uninfected[1:(.N-1)] - uninfected_jail_pop[1:(.N-1)])))),by = ycat]
  setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
  

summarize_yearly_jailed_percentage  <- function(filename = "counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  jailed_pc <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(jail_pop)/(vertex_count))),by = ycat]
  setnames(jailed_pc,c("V1","V2"),c("mean","sd"))
}

summarize_yearly_ever_jailed_percentage  <- function(filename = "counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  ever_jailed_pc <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(vertex_count_ever_jailed)/(vertex_count))),by = ycat]
  setnames(ever_jailed_pc,c("V1","V2"),c("mean","sd"))
}

summarize_yearly_jailed_percentage_hiv_pos  <- function(filename = "counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  jailed_pc_hiv_pos <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(jail_pop - uninfected_jail_pop)/(vertex_count- uninfected))),by = ycat]
  setnames(jailed_pc_hiv_pos,c("V1","V2"),c("mean","sd"))
}

summarize_yearly_jailed_percentage_hiv_neg  <- function(filename = "counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  jailed_pc_hiv_neg <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(uninfected_jail_pop)/(uninfected))),by = ycat]
  setnames(jailed_pc_hiv_neg,c("V1","V2"),c("mean","sd"))
}


summarize_yearly_inc_ever_jailed <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_ever_jailed[2:.N])/(uninfected_ever_jailed[1:(.N-1)]))),by = ycat]
  setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
 
 summarize_yearly_inc_never_jailed <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
 ...
 summarize_yearly_inc_nojailed_partners <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_via_transmission[2:.N] - infected_jailed_partner[2:.N])/(uninfected[1:(.N-1)] - uninfected_jailed_partner[1:(.N-1)]))),by = ycat]
  setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
 
 summarize_yearly_inc_sub <- function(filename="counts.csv", ranges, interv_start_at=0){
   dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
   full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_via_transmission_substance_user[2:.N])/(pop_substance_users[1:(.N-1)] - pop_infected_substatnce_user[1:(.N-1)]))),by = ycat]
   setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
 
 summarize_yearly_inc_nosub <- function(filename="counts.csv", ranges, interv_start_at=0){
   dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
   full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_via_transmission_non_substance_user[2:.N])/(vertex_count[1:(.N-1)] -pop_substance_users[1:(.N-1)]-pop_infected_substatnce_user[1:(.N-1)]))),by = ycat]
   setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
 
 summarize_yearly_prev_sub  <- function(filename = "counts.csv", ranges, interv_start_at=0){
   dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
   prev <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(pop_infected_substatnce_user)/(pop_substance_users))),by = ycat]
   setnames(prev,c("V1","V2"),c("full_inc_mean","sd"))
 }
 
 summarize_yearly_prev_nosub  <- function(filename = "counts.csv", ranges, interv_start_at=0){
   dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
   prev <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(pop_infected_non_substance_user)/(vertex_count-pop_substance_users))),by = ycat]
   setnames(prev,c("V1","V2"),c("full_inc_mean","sd"))
 }
 

 summarize_yearly_cases_meth <- function(filename="counts.csv", ranges, interv_start_at=0){
   dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
   tot_inc <- dt_counts[,lapply(c(sum), function(x) x((infected_via_transmission_meth))),by = ycat]
   setnames(tot_inc,c("V1"),c("tot_inc_sum"))
 }
 
 
 
summarize_yearly_inc <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
