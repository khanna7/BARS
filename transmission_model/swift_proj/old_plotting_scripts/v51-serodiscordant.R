...
full_res <- read.csv("../data/v51-serodiscordant-final_results.csv", header=FALSE) #final results
loc <- "../../v51-serodiscordant"
...
#saveRDS(object = dt, file = "out/v51-serodiscordant.RDS")
dt <- readRDS("out/v51-serodiscordant.RDS")
...
p2 <-
  ggplot(filter(all_summ_inc_dt, factor(param_set) %in% as.factor(1:6)), 
         aes(x=ycat, y=avg_annual_inc, color=final_uptake))+
  geom_line()+geom_point()+
  ggtitle("Average PrEP Retention = 1 year")+
  xlab("year of intervention")+
