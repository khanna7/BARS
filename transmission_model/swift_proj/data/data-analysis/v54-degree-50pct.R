...
full_res <- read.csv("../data/v54-50pct-degree-final_results.csv", header=FALSE) #final results
loc <- "../../v54-50pct-degree"

in1 <- read.table(paste0(loc,"/instance_1/output/parameters.txt"))
upf <- read.table(paste0(loc, "/50pct-degree.txt"), sep=",")
...
# Pull out degreevector results with top 10% of degree scorers targeted ----

intrv.prep.topn.0.1 <- which(upf$V6 == "degree.intrv.prep.topn=0.1")
...
saveRDS(object = dt, file = "out/v54-50pct-degree.RDS")
dt <- readRDS("out/v54-50pct-degree.RDS")
...
n.scen <- 5 #ONLY for testing top 10%
#prev.dt <- dt[["prevs"]]
