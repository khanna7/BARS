...
full_res <- read.csv("../data/v54-random-selection-final_results.csv", header=FALSE) #final results
loc <- "../../v54-random-selection"

in1 <- read.table(paste0(loc,"/instance_1/output/parameters.txt"))
upf <- read.table(paste0(loc, "/random-selection.txt"), sep=",")
...
#saveRDS(object = dt, file = "out/v54-random-selection-final_results.RDS")
dt <- readRDS("out/v54-random-selection-final_results.RDS")
...
saveRDS(inc.dt.10$full_inc_mean, file = "out/v54-random-selection-full_inc_mean.RDS")
