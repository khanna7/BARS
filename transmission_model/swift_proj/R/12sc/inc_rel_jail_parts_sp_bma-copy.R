...
summarize_yearly_prev_never_jailed  <- function(filename = "counts_2.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)  
  prev <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(vertex_count_never_jailed - uninfected_never_jailed)/(vertex_count_never_jailed))),by = ycat]
  setnames(prev,c("V1","V2"),c("mean","sd"))
}
...
for (j in 1:sc_nb){
        ...
        for(i in next_sc:(next_sc+rseed_nb-1)){              
				...
				#res2.dt   <- summarize_yearly_prev_never_jailed(paste0(dir_loc,"/output/counts_2.csv"), range, interv_start_at=year_interv_start)
...
t13<-inc_means_melt.df[c(31:40),]
#t13<-inc_means_melt.df[c(31:40,71:80,111:120),]

t13<-inc_means_melt.df[c(31:40,71:80,111:120,151:160,191:200),]
...
t16<-inc_means_melt.df[c(31:40,191:200),]


names(t16)[c(4,5)]<-c("Two","NinetySeven")
#ribbon1<-apply(matrix(unlist(quantiles2.597.5),ncol=1000),1,quantile,c(.025,.975),na.rm=TRUE)

#t13<-inc_means_melt.df[c(31:40,71:80,111:120,151:160,191:200),]
#t13$series<-c(rep("90",5),rep("180",5),rep("270",5),rep("360",5),rep("720",5))
t17<-t17[c(11:50),]

t15<-t15[-c(21:30,41:50),]
names(inc_means_melt.df)[c(4,5)]<-c("Two","NinetySeven")
t15<-rbind(t15,inc_means_melt.df[31:40,])
t15$series[c(31:40)]<-720
t15$time<-rep(1:10,4)
t15$series<-c(rep("2.Minimal Disruption",10),rep("3.Targeted and Sustained HIV Care",10),rep("1.Baseline (Out of Care for 3 Months)",10),rep("4.Out of Care for 24 Months",10))
ggplot(t15, aes(x = time,  color = series, fill = series)) + 
        xlab("Year") +
        ylab("Average Incidence Rate") +
        labs(title="Post Release Partners") +
        ylim(0, 20) +
