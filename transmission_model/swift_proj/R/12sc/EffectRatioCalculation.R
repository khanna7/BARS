netbase<-do.call(rbind,list_inc)[1:4*40,]
save(netbase,file="CareEffectRatiosBaseGen.Rdata")
#General
...
for(i in 1:4){
  a[[i]]<-lapply(1:1000,function(u){sample(netbase[i,],30,replace=TRUE)})
...
d[4,]<-quantile((b[[4]][[]])/(b[[4]][[1]]),c(0.025,.975))
