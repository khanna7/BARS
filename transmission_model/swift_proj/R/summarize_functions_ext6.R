...
summarize_yearly_prev_ever_jailed1  <- function(filename = "counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)  
  prev <- dt_counts[,lapply(c(mean,sd), function(x) x(100*(vertex_count_ever_jailed)/(vertex_count))),by = ycat]
  setnames(prev,c("V1","V2"),c("mean","sd"))
}
...
summarize_yearly_inc_ever_jailed <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_via_transmission_ever_jailed[2:.N])/(uninfected_ever_jailed[1:(.N-1)]))),by = ycat]
  setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
 
 summarize_yearly_inc_never_jailed <- function(filename="counts.csv", ranges, interv_start_at=0){
  dt_counts <- create_dt_counts(filename,ranges, interv_start_at)
  full_inc <- dt_counts[,lapply(c(mean), function(x) x(365*100*(infected_via_transmission_never_jailed[2:.N])/(uninfected_never_jailed[1:(.N-1)]))),by = ycat]
  setnames(full_inc,c("V1"),c("full_inc_mean"))
 }
